...
public:
    ...
    typename RoomData<TDataType>::SingleMultyMembers addToRoom(const std::string &room_id, const Members &mbr) {
    ...
    typename RoomData<TDataType>::SingleMultyMembers deleteFromRoom(const std::string &room_id, const Members &mbr) {
        typename RoomData<TDataType>::SingleMultyMembers members;
        auto iter = Base::_rooms.find(room_id);
        if (iter not_eq Base::_rooms.end()) {
            size_t id_0 = std::get<0>(mbr);
