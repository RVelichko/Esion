...
BOOST_AUTO_TEST_CASE(TestDbFacade) {
    ...
    size_t num = 0;
    size_t found = 0;
    for (auto jdev : dbf.getList(found, CONTROOLERS_COLLECTION_NAME, "Debug Debug", "", "", 0, 0, "", true, 10, 2)) {
        LOG(DEBUG) << "[" << ++num << "] " << jdev;
    } 
    ...
    LOG(DEBUG) << "######################################";
    num = 0;
    for (auto jdev : dbf.getList(found, CONTROOLERS_COLLECTION_NAME, "Debug Debug", "", "", 0, 0)) {
        LOG(DEBUG) << "[" << ++num << "] " << jdev;
    } 
    dbf.disconnect();
