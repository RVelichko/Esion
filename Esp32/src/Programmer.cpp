...
bool Programmer::_rst_active_high = false;
...
void Programmer::sendReset(bool reset) {
    bool rst = ((reset and _rst_active_high) || (not reset and not _rst_active_high));
    #ifdef DEBUG
    Serial.println("Programmer set reset to " + String(rst ? "HIGH" : "LOW"));
    #endif
    digitalWrite(RESET, rst ? HIGH : LOW);
}
